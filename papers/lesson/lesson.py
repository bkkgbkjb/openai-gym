...
class LowCritic(NeuralNetworks):
    ...
    def forward(self, s: State, g: Goal, a: torch.Tensor) -> torch.Tensor:
        ...
        return self.net(torch.cat([s.to(DEVICE), g.to(DEVICE), a.to(DEVICE)], 1))
...
class LowNetwork(Algorithm):
    def __init__(self, state_dim: int, goal_dim: int, action_dim: int):
        ...
        self.critic_optim = torch.optim.Adam(
            self.critic.parameters(), lr=2e-4, weight_decay=1e-5
        )
...
class HighNetwork(Algorithm):
    def __init__(self, state_dim: int, goal_dim: int, action_dim: int) -> None:
        ...
        self.sac = NewSAC(self.state_dim + self.goal_dim, self.goal_dim, 1.0, True)
...
class LESSON(Algorithm):
    def __init__(self, state_dim: int, goal_dim: int, action_dim: int):
        ...
        self.low_network = LowNetwork(self.state_dim, self.goal_dim, self.action_dim)

        self.high_network = HighNetwork(self.state_dim, self.goal_dim, self.action_dim)
    ...
    def on_env_reset(self, info: Dict[str, Any]):
        self.desired_goal = (
            torch.from_numpy(info["desired_goal"]).type(torch.float32).to(DEVICE)
        )

        obs = torch.from_numpy(info["observation"]).type(torch.float32).to(DEVICE)
        self.representation_goal = (
            self.representation_network(obs.unsqueeze(0)).squeeze(0).to(DEVICE)
        )
    ...
    def take_action(self, state: State) -> Action:
        if self.inner_steps % self.c == 0:
            (act, info) = self.high_network.take_action(
                state.to(DEVICE), self.desired_goal, self.representation_goal
            )
        ...
        act = self.low_network.take_action(
            ...
            torch.from_numpy(self.current_high_act)
            .type(torch.float32)
            .unsqueeze(0)
            .to(DEVICE),
        )
    ...
    def collect_samples(
        ...
        hi_obs_next = [
            (s.state for s in e.get_steps(list(map(lambda _ts: _ts + self.c, ts))))
            for e in episodes
        ]
        obs_next = [
            (s.state for s in e.get_steps(list(map(lambda _ts: _ts + 1, ts))))
            for e in episodes
        ]
    ...
    def update_phi(self):
        ...
        max_dist = (1 - (new_hi_obs - new_hi_obs_next)).pow(2).mean(dim=1).clamp(min=0)
    ...
    def after_step(self, transition: TransitionTuple[State]):
        ...
        if (
            self.inner_steps != 0
            and self.inner_steps % self.c == self.c - 1
            and not s2.is_end()
        ):
            ...
            self.high_network.after_step(
                (
                    NotNoneStep(
                        self.last_high_obs, self.last_high_act, self.high_reward
                    ),
                    Step(
    ...
    def on_episode_termination(
        ...
        self.high_network.after_step(
            (
                NotNoneStep(self.last_high_obs, self.last_high_act, self.high_reward),
                Step(
